...
namespace Chen.DB
{
    ...
    public class DbCommon
    {
        ...
        public static void CreateHtml(DataTable dt, bool KeepNull, string Path, bool hasReturn = true, string tableDesc = "")
        {
            ...
            code.AppendLine("    <META http-equiv=\"Content-Type\" content=\"text/html; charset=utf-8\"> ");
            ...
            File.WriteAllText(Path, code.ToString(), Encoding.GetEncoding("utf-8"));
            //File.WriteAllText(Path, code.ToString(), Encoding.UTF8);
        }

        public static string CreateHtmlAllTable(List<DataTable> listDt, bool keepNull, string path, string fileName, bool hasReturn = true, string tableDesc = "")
        {
            var code = new StringBuilder();
            code.AppendLine("<!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.0 Transitional//EN\" \"http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd\">");
            code.AppendLine("<html xmlns=\"http://www.w3.org/1999/xhtml\">");
            ...
            foreach (var dt in listDt)
            {
                ...
                dr["序号"] = tableIndex++;
                dr["表名"] = "<a href=\"#{0}\">{0}</a>".FormatString(dt.TableName);
                dtMenus.Rows.Add(dr);
            }
            code.AppendLine(CreateOneTableHtml(dtMenus, false, false));

            foreach (var dt in listDt)
            {
                code.AppendLine(CreateOneTableHtml(dt, true));
            }
            ...
            File.WriteAllText(path, code.ToString(), Encoding.GetEncoding("utf-8"));
            return "导出完毕。 存储位置为：" + path;
        }

        public static string CreateOneTableHtml(DataTable dt, bool KeepNull, bool hasReturn = true, string tableDesc = "")
        {
            string tableName = hasReturn ? dt.TableName : "tableMenu";
            var code = new StringBuilder();
            code.AppendLine("    <div style=\"text-align: center\">");
            code.AppendLine("        <div>");
            code.AppendLine("            <table id=" + tableName + " border=\"0\" cellpadding=\"5\" cellspacing=\"0\" width=\"90%\">");
            code.AppendLine("                <tr>");
            code.AppendLine("                    <td bgcolor=\"#FBFBFB\">");
            code.AppendLine("                        <table cellspacing=\"0\" cellpadding=\"5\" border=\"1\" width=\"100%\" bordercolorlight=\"#D7D7E5\" bordercolordark=\"#D3D8E0\">");
            code.AppendLine("                        <caption>");
            code.AppendLine("        <div class=\"styletab\">{0}{1}{2}</div>".FormatString(
                dt.TableName,
                tableDesc.Length == 0 ? string.Empty : "  （" + tableDesc + "） ",
                hasReturn ? "<a href='#tableMenu' style='float: right; margin-top: 6px;'>返回目录</a>" : string.Empty));
            code.AppendLine("                        </caption>");
            code.AppendLine("                        <tr bgcolor=\"#0094FF\" color=\"#FFF\">");
            //构建表头
            foreach (DataColumn dc in dt.Columns)
            {
                code.AppendLine("            <td>{0}</td>".FormatString(dc.ColumnName));
